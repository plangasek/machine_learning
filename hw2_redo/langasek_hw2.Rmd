...
grad_rec <- function(X, iterations) {
  set.seed(1235)
  X <- cbind(1, X)
  w = matrix(0, ncol = ncol(housing_train), nrow = 1 ) #14 columns: 1 intercept and 13 parameter weights
  ...
  for (i in 1:iterations) {
    ...
    gradient_vals <- (1 / N) * (current_row[15] - t(w) * current_row[1:14]) * current_row[1:14]
    w <- w + alpha * t(gradient_vals) # update weights from this sample.
...
This is giving me a constant value of 22.08, which means something is deeply amiss.

```{r}
#plot(housing_MSE_graph, y )
```
